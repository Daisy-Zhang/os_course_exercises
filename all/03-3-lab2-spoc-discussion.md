...
2. 不同特权级有什么区别？
   ...
   答：不同特权级权限不同，能使用的特权指令不同，特权级`0-3`中`0`优先级最高，`3`优先级最低。

3. 请说明CPL、DPL和RPL在中断响应、函数调用和指令执行时的作用。

   答：

   * CPL：标注了当前优先级，用作判断优先级可否访问
   * RPL：标注了要求的优先级，用于判断访问的优先级是否达到要求
   * DPL：描述符优先级，表示门或者段的优先级，当去访问门或者段时，会将该优先级与当前优先级进行比较
...
3. int指令在ring0和ring3的执行行为有什么不同？
   ...
   答：因为ring0和ring3能够调用的特权指令和访问的数据不同。

4. 如何利用int和iret指令完成不同特权级的切换？

   答：发生特权级转换时，会构建一个新栈，然后调用`int`指令时会将原栈中相关信息压栈，将中断标号压栈，调用`iret`指令返回时再将新栈中各个信息取出。

5. TSS和Task Register的作用是什么？

 > [Task state segment](https://en.wikipedia.org/wiki/Task_state_segment)：存储任务的信息，用于任务(进程)调度
...
1. 一条指令执行时最多会出现多少次地址转换？
   ...
   答：一条指令最多涉及三操作数，即三块内存空间的访问，每次地址涉及`va -> la -> pa`的转换，故一条指令最多出现`6`次地址转换。

2. 描述X86-32的MMU地址转换过程。

   答：`x86-32`的机器上，`MMU`分为两级页表结构，第一级为页目录表`PDT`，第二级为页表项`PT`，按照虚拟(线性)地址的位划分去依次查询以上两级页表，最后得到物理地址。
...
2. 分析页表的建立过程。
   ...
   * 首先定义好了相关分配内存的算法，lab2中为`FIFA`
   * 然后探测物理内存，并创建`page list`
   * 对上述构建完成的`list`进行检查
   * 将`物理地址`和`虚拟地址`映射起来
...
* 初始化`GDT`构建段机制
* 探测物理空间并进行物理内存管理(如分配算法和碎片处理，已经使用内存的管理策略)
* 构建并使能页表，建立页机制
