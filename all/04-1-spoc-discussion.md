...
2. 如何理解计算机中的存储层次结构所的理想状态是“均衡繁忙”状态？
   ...
   > 均衡繁忙状态：存储结构均处于繁忙状态，但不构成系统瓶颈

3. 在你写程序时遇到过内存不够的情况吗？尝试过什么解决方法？

   > 遇到过，尝试过尽量优化程序内的内存使用，及时释放
...
2. 什么是交换技术？覆盖与交换有什么不同？
   ...
   > 交换技术：将内存中暂时用不到的空间换入外存，需要用到时再换入
   >
   > 覆盖技术：使可用空间大于物理空间
   >
   > 交换技术：将暂时不需要的进程内存换出到外存，让正在或者将要执行的程序有足够内存

3. 覆盖和交换技术在现代计算机系统中还有需要吗？可能用在什么地方？

   > 应该还有部分计算机用到了这些技术，主要用在解决物理内存不够的情况
...
1. 什么是时间局部性、空间局部性和分支局部性？
   ...
   > 时间局部性：访问某一内存的时间集中在一小范围内
   >
   > 空间局部性：若访问了某一地址的内存，则该地址及附近地址内存很可能接下来再次被访问
   >
   > 分支局部性：某一条跳转指令的两次执行，很可能跳转到同一地址内存

2. 如何提高程序执行时的局部性特征？

   > 程序访问数据时，尽可能访问处于相近地址空间的数据
   >
   > 程序访问某一数据时，集中访问
   >
   > 程序跳转指令时，同一跳转，目标尽可能相同
...
1. 什么是虚拟存储？它与覆盖和交换的区别是什么？它有什么好处和挑战？
   ...
   > 虚拟存储：只把部分程序放到内存中，从而运行比物理内存大的程序；实现内存和外存之间的交换，获得更大的存储空间
   >
   > 好处：大地址空间
   >
   > 挑战：地址映射技术，不连续性，部分交换存储
...
### 8.5 虚拟页式存储
 ...
 1. 什么是虚拟页式存储？缺页中断处理的功能是什么？
    ...
    > 虚拟页式存储即不会一次把程序所有空间读入内存，而是分页读入，其他的暂时放在外存，等到需要时再把其置换进内存。
    >
    > 缺页中断处理：把外存中的某一页内容加载进内存中的某一物理
 ...
 1. 页式存储和虚拟页式存储的区别是什么？
    ...
    > 页式存储指的是将数据或者代码存入内存的方式为分页存入
    >
    > 虚拟页式存储指的是利用页表项完成虚拟地址访存操作
...
2. 虚拟页式存储管理中有效存储访问时间是如何计算的？
   ...
   > 设缺页率为`p`，`EAT  = 访问内存时间 * (1-p) + 缺页处理时间 * P`
