...
> `do_fork()`函数返回的是创建的子进程被分配的`pid`，而父子进程调用`do_fork()`时给子进程的`pid`是由`get_pid()`函数返回的，所以父子进程返回的不同

3. 内核线程initproc的第一次执行流程是什么样的？能跟踪出来吗？

> 线程通过`fork()`创建，`load_icode()`加载二进制程序后运行，`exit`退出进程
...
> 调用汇编语言`switch_to()`函数时传参为前后两个进程的寄存器现场值，通过寄存器来传递
...
8. 分析内核线程initproc的创建流程，说明线程切换后执行的第一条是什么。
 ...
 > 在`idleproc`进程创建之后创建的`initproc`进程，通过`proc_run`执行并切换进程，切换进程后执行第一条应该是使能恢复中断
